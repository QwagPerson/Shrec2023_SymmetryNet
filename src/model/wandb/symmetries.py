import numpy as np
from src.model.wandb.transformations import *
...
class SymmetryAxis:
    ...
    def __repr__(self):
        ...
        #return (f"SymmetryAxis(point={self.point.tolist()}, normal={self.normal.tolist()}, angle={self.angle})")
        return (f"SymmetryAxis(point={np.round(self.point.astype(np.float64), 3).tolist()}, normal={np.round(self.normal.astype(np.float64), 3).tolist()}, angle={np.round(self.angle, 3)})")
...
class SymmetryPlane:
    ...
    def __repr__(self):
        #return (f"SymmetryPlane(point={self.point.tolist()}, normal={self.normal.tolist()})")
        return (f"SymmetryPlane(point={np.round(self.point.astype(np.float64), 3).tolist()}, normal={np.round(self.normal.astype(np.float64), 3).tolist()})")
