...
# Script
# python model_exploration.py ckpt config split outputpath
# export csv with
# everything per type of symmetry
# loss,map,phc => metrics
# metrics in 3 tolerances (1°, 1%), (3°, 3%), (5°, 5%) total and per perturbance/shape/symtype
# around 150 metrics!
# head_hits
...
if __name__ == "__main__":
    ...
    for shape_type_text, shape_type_idx in SHAPE_TYPE.items():
        ...
        print(f"{f'Plane Symmetry MAP {shape_type_text}:':<45}{shape_map.item():.3f}")
    ...
    for perturbation_text, perturbation_idx in PERTURBATION_TYPE.items():
        ...
        print(f"{f'Plane Symmetry MAP {perturbation_text}:':<45}{perturbation_map.item():.3f}")
